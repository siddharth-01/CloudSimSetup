...
public class Host {
	...
	private Integer utilizationOfCPU;
	
	private boolean isUnderLoaded = false;
	
	private boolean isOverLoaded = false;
	
	private Integer performaceToPowerRatio = 20;
	
	private String hostType ;
	
	private List<Vm> virtualMachineList = new ArrayList<>();
	...
	public Integer getLevelOfObservedMaxPPR() {
	Integer overLoadThreshold =  (this.powerConsumption - this.utilizationOfCPU) * this.performaceToPowerRatio ;
		return overLoadThreshold;
	}

	public Integer getLowerThreshold() {
		System.out.println("powerConsumption = " + this.powerConsumption + " " + "cpu utilization = " + this.utilizationOfCPU);
		Integer lowerThreshold =  (this.powerConsumption - this.utilizationOfCPU) ;
		System.out.println("lowerThreshold  = " + lowerThreshold);
	...
	public Integer setUtilizationOfCPU() {		
		if(this.hostType == "HpProLiantML110G5") {
			this.utilizationOfCPU = 300;
		} else if(this.hostType == "HpProLiantML110G4") {		
			this.utilizationOfCPU = 200;		
		}
		else if(this.hostType == "HPProLiantDL360G7") {		
			this.utilizationOfCPU = 1995;		
		}
		else if(this.hostType == "HPProLiantDL360G9") {		
			this.utilizationOfCPU = 2495;		
		}
		return 0;
	}
	
	public Integer afterUtilization() {
		return 1400;
	}
	
	public Integer estimatePowerAfterAllocation() {
		return 1000;
	}
	
	
	public Integer getUtilizationOfCPU() {		
		return this.utilizationOfCPU;
	}
	...
	public void createVMList(List<Vm> vmlist) {
			this.virtualMachineList.addAll(vmlist);	

	}
	
	public List<Vm> getVirtualMachineList() {
	...
	public double averageEnergyConsumption() {
		...
		for(Vm vm : this.getVirtualMachineList()) {
			sumOfEnergyConsumption = sumOfEnergyConsumption + vm.getEnergyConsumption();
		}
		
		return (sumOfEnergyConsumption)/this.getVirtualMachineList().size();
